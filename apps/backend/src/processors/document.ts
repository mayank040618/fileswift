...
const compressPdfProcessor: ToolProcessor = {
    id: 'compress-pdf',
    process: async ({ job, localPath, outputDir }) => {
        // Minimal compression using pdf-lib (re-save)
        // For better compression, we would need ghostscript
        const pdfBytes = await fs.readFile(localPath);
        const pdfDoc = await PDFDocument.load(pdfBytes);

        // Use standard options for "compression" like logic
        const outputBytes = await pdfDoc.save({ useObjectStreams: false }); // sometimes larger, sometimes smaller. 
        // Actually, pdf-lib doesn't compress much. Let's just pass-through with new ID for MVP.

        const outputFilename = `compressed-${job.id}.pdf`;
        const outputPath = path.join(outputDir, outputFilename);
        await fs.writeFile(outputPath, outputBytes);
